...
from master_utils import build_columns, record_filename, eval_record_filename, append_row_to_workbook, upsert_row_in_summary, build_row_dict, aggregate_score
...
def resolve_box(field_or_option, page_images, template, zone_transform_cache):
    """
    取得欄位實際要裁切的座標。
...
def main():
    ...
    if uploaded_pdf is not None:
        ...
        for idx, record in enumerate(records):
            ...
            with st.expander(f"第 {idx + 1} 份 / 共 {n} 份 —— {status}", expanded=not record["saved"]):
                ...
                if record["saved"]:
                    ...
                    st.session_state["_records"][idx]["saved_data"] = edited
        ...
        if all(r["saved"] for r in records):
            ...
            if form_type_name == "供應商評核表" and n > 1:
                st.divider()
                st.subheader("📊 多單位評分彙整平均")
                st.caption(
                    "把上面每個單位各自填的分數取平均，算出這次評核的總分。"
                    "⚠️ 目前的平均算法是：每個評核項目(a1~d3)取「有填寫的單位」的得分平均，"
                    "10 個項目的平均分數加總為總分(滿分100)，加分項目(e1、e2)同樣取平均、每項上限10分再加進總分，"
                    "等級依表單上寫的門檻(90分以上A級、70~89分B級、其餘C級)判定。"
                    "**這個計算規則是我依表單上的文字說明推測的，請務必確認是否符合貴公司實際的計算方式，如果不對請告訴我怎麼調整。**"
                )

                all_data = [r["saved_data"] for r in records]
                agg = aggregate_score([uploaded_pdf.name] * n, all_data)
                ...
                agg_key = f"_agg_saved_{cache_key}"
                if st.session_state.get(agg_key):
                    st.info("彙總分數已經存檔過了。")
                elif st.button("✅ 確認並存彙總分數", type="primary"):
                    with st.spinner("寫入 Google Drive 中..."):
                        drive_folder_id = st.secrets["drive_folder_id"]
                        service = get_drive()
                        supplier_folder_id = find_or_create_folder(service, drive_folder_id, form_cfg["supplier_subfolder"])

                        row = build_row_dict(f"{uploaded_pdf.name}(彙總)", agg)
                        fname = filename_fn(agg)
                        existing_id = find_file_id(service, supplier_folder_id, fname)
                        existing_bytes = download_file_bytes(service, existing_id) if existing_id else None
                        new_bytes = append_row_to_workbook(existing_bytes, columns, row)
                        upload_or_update_xlsx(service, supplier_folder_id, fname, new_bytes)
