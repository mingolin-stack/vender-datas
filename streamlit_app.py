...
def main():
    ...
    uploaded_pdf = st.file_uploader(f"② 上傳已掃描的{form_type_name}(PDF，一次一份)", type=["pdf"])
    ...
    if uploaded_pdf is not None:
        ...
        if "_suggestions" not in st.session_state:
            with st.spinner("辨識中，請稍候(掃描頁面較多或網路較慢時可能需要一點時間)..."):
                ...
                suggestions, crops = run_extraction(page_images, template, vision_client)
        ...
        if st.button("✅ 確認並存檔", type="primary"):
            ...
            st.balloons()
            for k in ["_suggestions", "_crops", "_current_key"]:
